...
public class LCommand implements CommandExecutor {
    ...
    public LCommand(MsgPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(final CommandSender sender, Command command, String label, String[] args) {
        ...
        if(plugin.getLastSent(sender.getName()) == null){
            sender.sendMessage(ChatColor.GRAY + "You have no one to message.");
            return true;
        }

        Player receiver = Bukkit.getPlayer(plugin.getLastSent(sender.getName()).getLastSent());
